...
export const ScrambleWords = () => {
  ...
  const handleGuessSubmit = (e: React.FormEvent) => {
    ...
    if (guess === currentWord) {
      ...
      setPoints(points + 1);
      const remainingWords = words.filter((_, index) =>
        index !== 0);
      setWords(remainingWords);

      if (remainingWords.length > 0) {
        const nextWords = remainingWords[0];
        setCurrentWord(nextWords);
        setScrambledWord(scrambleWord(nextWords));
      }
      setGuess("");
    }
  ...
  const handleSkip = () => {
    ...
    const remainingWords = words.filter((_, index) => index !== 0
    );
    setWords(remainingWords);
